...
def _calculate_metrics(all_labels, all_preds, all_probs, total_loss, num_batches):
    ...
    is_binary = labels_set.issubset({0, 1}) and preds_set.issubset({0, 1})
    ...
    tn, fp, fn, tp = confusion_matrix(all_labels, all_preds, labels=[0, 1]).ravel() if is_binary else [0, 0, 0, 0]
